...
function getMachineInfo(id) {
	...
	return isLocal ? mockAjax(fakeData) : getData(machineApiUrl + '/' + id, data);
};
...
function getUserList() {
	...
	return isLocal ? mockAjax(fakeData) : getData(config.APIUrl + 'user/list', data);
}

function getAvailabilityRate(id) {
	var fakeData = 100;
	return isLocal ? mockAjax(fakeData) : getData(machineApiUrl + '/availability_rate/' + id, data);
}
